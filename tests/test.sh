...
do
	## run simuls
	echo "Let's test the simulations with stochastic logistic growth"
	...
	--save_all TRUE \
	--outputname result_X$sa --outdir $RESULTSDIR/$OUTPUTPREFIX$sa/new --cores $CORES

	## run simuls (old; no logistic growth, output's separated by PCG)
        echo
        echo "Let's test the old function (no logistic growth, output's separated by PCG)"
